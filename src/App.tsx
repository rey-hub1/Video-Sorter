...
export default function App() {
    ...
    const getUserStats = (user: string) => {
        ...
        const done = userSegment.filter((v: VideoData) => v.kategori).length;
        const percentage = Math.round((done / userSegment.length) * 100);
        return { done, total: userSegment.length, percentage };
    };
    ...
    const handleUserSelect = async (user: string) => {
        ...
        try {
            ...
            const firstUncategorized = partitionedVideos.findIndex(
                (v) => !v.kategori,
            );
            ...
            setError(
                `Gagal mengambil data: ${(err as Error).message}. Pastikan URL Webhook sudah benar dan workflow n8n sudah Active.`,
            );
        } finally {
            setIsLoading(false);
    ...
    const handleCategoryClick = async (category: CategoryData) => {
        ...
        const nextUncategorized = updatedVideos.findIndex(
            (v, idx) => idx > currentIndex && !v.kategori,
        );
        if (nextUncategorized !== -1) {
            setCurrentIndex(nextUncategorized);
        } else {
            // If none after, check from start of user's list
            const firstUncategorized = updatedVideos.findIndex(
                (v) => !v.kategori,
            );
    ...
    const currentVideo = userVideos[currentIndex];
    const stats = useMemo(() => {
        const done = userVideos.filter((v: VideoData) => v.kategori).length;
    ...
    const handleAddCategory = () => {
        if (newCatName.trim()) {
            const exists = tempCategories.some(
                            ...
                            >
                                {selectingUser === user ? (
                                    <div className="flex flex-col items-center gap-3">
                                        ...
                                        <span className="text-[10px] md:text-xs font-black text-indigo-400 uppercase tracking-[0.2em] animate-pulse">
                                            Memuat Data...
                                        </span>
                                    </div>
                                ) : (
                                    <>
                                        <div
                                            className={`w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-slate-800 flex items-center justify-center group-hover:bg-indigo-500 transition-colors overflow-hidden relative ${isLoading ? "opacity-50" : ""}`}
                                        >
                                            <span className="text-xl md:text-2xl font-black text-slate-400 group-hover:text-white uppercase transition-colors z-0">
                                                {user.charAt(0)}
                                            </span>
                                            <img
                                                src={`/${user.toLowerCase()}.jpg`}
                                                alt={user}
                                                ...
                                                onError={(e) => {
                                                    ...
                                                    if (
                                                        ...
                                                        target.src = `/${user.toLowerCase()}.png`;
                                                    } else if (
                                                        target.src.includes(
                                                            ".png",
                                                        )
                                                    ) {
                                                        target.src = `/${user.toLowerCase()}.jpeg`;
                                                    } else {
                                        ...
                                        <span
                                            className={`text-sm md:text-lg font-black text-white ${isLoading ? "opacity-50" : ""}`}
                                        >
                                            {user}
                                        </span>
                ...
                <div className="max-w-2xl mx-auto space-y-8">
                    <div className="flex items-center justify-between">
                        <h1 className="text-2xl font-black tracking-tighter text-white">
                            MANAJEMEN{" "}
                            <span className="text-indigo-500">KATEGORI</span>
                        </h1>
                    ...
                    <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 space-y-8">
                            ...
                            <button
                                onClick={() => {
                                    ...
                                    if (newCatName.trim()) {
                                        ...
                                        if (!exists) {
                                            const maxId = tempCategories.reduce(
                                                (max, cat) => {
                                ...
                                className="px-6 py-4 sm:py-auto h-full min-h-12.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-black text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-2 self-start sm:self-stretch mt-auto sm:mt-0 w-full sm:w-auto"
                            >
                        ...
                        <div className="space-y-4">
                            {tempCategories.map((cat, idx) => (
                                <div
                                    key={cat.id || idx}
                                    className="flex flex-col sm:flex-row items-center gap-3 group bg-slate-950/20 p-3 rounded-2xl border border-slate-800/50 hover:border-slate-800 transition-colors"
                                >
                                    ...
                                    <button
                                        ...
                                        className="hidden sm:flex w-12 h-full min-h-20 bg-slate-900 border border-slate-800 rounded-xl items-center justify-center text-slate-500 hover:bg-red-500/10 hover:border-red-500/20 hover:text-red-500 transition-colors shrink-0"
      ...
      `}
            >
                <div className="p-6 border-b border-slate-800 bg-indigo-600/10">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-lg bg-slate-800 flex items-center justify-center overflow-hidden relative shrink-0">
                                <span className="text-sm font-black text-slate-400 uppercase z-0">
                                    {activeUser?.charAt(0)}
                                </span>
                                {activeUser && (
                                    <img
                                        src={`/${activeUser.toLowerCase()}.jpg`}
                                        alt={activeUser}
                                        className="absolute inset-0 w-full h-full object-cover z-10"
                                        onError={(e) => {
                                            const target = e.currentTarget;
                                            if (target.src.includes(".jpg")) {
                                                target.src = `/${activeUser.toLowerCase()}.png`;
                                            } else if (
                                                target.src.includes(".png")
                                            ) {
                                                target.src = `/${activeUser.toLowerCase()}.jpeg`;
                                            } else {
            ...
            <main className="flex-1 flex flex-col min-w-0 h-full">
                <header className="h-16 border-b border-slate-800 flex items-center justify-between px-4 md:px-8 shrink-0 bg-slate-950/50 backdrop-blur-md sticky top-0 z-30">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col min-w-0">
                            <h3 className="text-xs md:text-sm font-black text-white truncate max-w-37.5 sm:max-w-md">
                ...
                <div className="flex-1 p-4 md:p-8 flex flex-col gap-4 md:gap-8 overflow-y-auto">
                    ...
                    <div className="flex flex-col md:flex-row items-center gap-4 md:gap-6 flex-1 min-h-0">
                        ...
                        <div className="flex-1 w-full aspect-video min-h-62.5 md:min-h-0 bg-black rounded-2xl md:rounded-[2.5rem] overflow-hidden border border-slate-800 shadow-2xl relative md:max-h-[70vh]">
                                ...
                                {stats.total > 0 &&
                                    stats.done === stats.total && (
                                        <motion.div
                                            ...
                                            className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-slate-900 bg-[radial-gradient(ellipse_at_center,var(--tw-gradient-stops))] from-indigo-900/40 via-slate-900 to-slate-900 p-8 text-center"
                                        >
                                            ...
                                            <p className="text-slate-400 text-sm max-w-sm">
                                                ...
                                                penyortiran untuk semua daftar
                                                video yang ditugaskan kepada
                                                Anda saat ini.
                                            </p>
                                        </motion.div>
                                    )}
                            ...
                            >
                                ...
                                >
                                    ...
                                    <div
                                        className={`absolute inset-0 flex items-center justify-center bg-slate-900/50 ${iframeLoaded[currentVideo?.["ID File"] || ""] ? "hidden" : "block"}`}
                                    >
                        ...
                        <div className="flex md:hidden w-full justify-center items-center gap-2">
                            ...
                            <div className="px-3 py-1 bg-slate-800/50 rounded-full text-[8px] font-black text-slate-500 uppercase tracking-widest">
                                NAVIGASI
                            </div>
                            <button
                                onClick={goToNext}
